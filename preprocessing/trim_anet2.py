...
train_path = rep + 'aNet_trim2/train/'
val_path = rep + 'aNet_trim2/validation/'
test_path = rep + 'aNet_trim2/test/'
...
for item in train_list:
    ...
    for segment in db[item[0]]['annotations']:
        ...
        #print(segment['label'], start, end)
        
        dst = dst_path + 'v_' + item[0] + '_' + str(seg_idx) + ext

        if os.path.isfile(dst):
            exist = dst + " already exist!!"
            print(exist)
            continue

        print("Trimming training set from [" + src + "]" + " to " + "[" + dst + "], " + 
            "time[" + str(start) + ":" + str(end) + "], " + "duration[" + str(duration) + "]")

        if start > end:
            print("Sement time error from " + src)
            print("time[" + str(start) + ":" + str(end) + "], " + "duration[" + str(duration) + "]")
...
for item in val_list:
    ...
    for segment in db[item[0]]['annotations']:
        ...
        print("Trimming valdiation set from [" + src + "]" + " to " + "[" + dst + "], " + 
            "time[" + str(start) + ":" + str(end) + "], " + "duration[" + str(duration) + "]")
